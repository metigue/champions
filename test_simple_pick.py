#!/usr/bin/env python3
"""
Simple test to isolate the pick command output issue
"""
...
class SimpleCommandHandler:
    ...
    def pick_champions_for_battlegrounds(self, count: int, champion_names: str) -> str:
        ...
        for i, (champion, score, has_bg_rating) in enumerate(selected_champions, 1):
            ...
            # Only show battlegrounds information if the champion has a battlegrounds rating
            if champion.rating is not None:
                rating_str = f"{champion.rating}/10"
                ...
                # For champions without battlegrounds rating, just show that they're not good enough
                response += f"   - Not good enough to be on the tier list\n"
            response += f"   - Tier: {champion.tier}\n\n"
        
        return response

def test_simple_pick():
    """Test the simple pick functionality"""
    print("Testing simple pick functionality...")
    ...
    # Test case with champions that have and don't have ratings
    test_champions = "hulk, spider-man, wolverine, thor, karnak"
    print(f"\n=== Testing with: {test_champions} ===")
    result = command_handler.pick_champions_for_battlegrounds(5, test_champions)
    print(result)
